...
;(function(root, factory) {
    ...
    function writeJSON(data, idFile) {
        ...
        // setContent(file, content, tryCount)
       file.setContent(JSON.stringify(data,null,4))
    }
  
    function setContent(file, content, tryCount) {
    ...
    function expBackoff(tryCount) {
      var MAX_WAIT_TIME = 32000
      var waitTime = Math.min(getExpWaitTime(tryCount), MAX_WAIT_TIME)
      Utilities.sleep(waitTime)
    }
    
    function getExpWaitTime(tryCount) {
      return (Math.pow(2,tryCount) * 1000) + Math.floor(Math.random() * 1000)
    }
